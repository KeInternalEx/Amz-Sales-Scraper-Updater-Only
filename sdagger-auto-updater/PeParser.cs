...
namespace sdagger_auto_updater
{
    class PeParser
    {
        ...
        private static T ByteArrayToStructure<T>(byte[] Data) where T : struct
        {
            ...
            try { Structure = (T)Marshal.PtrToStructure(Handle.AddrOfPinnedObject(), typeof(T)); }
            finally { Handle.Free(); }
        ...
        public static uint AlignUpBy(uint Value, uint AlignTo)
        {
            if (Value % AlignTo != 0)
        ...
        public uint GetOverlayOffset()
        {
            int SectionHeadersOffset =
                this.DosHeader.e_lfanew +
                Marshal.SizeOf(typeof(IMAGE_NT_HEADERS32));

            int SectionHeaderSize = Marshal.SizeOf(typeof(IMAGE_SECTION_HEADER));
            int TargetHeaderOffset = SectionHeadersOffset +
                SectionHeaderSize * (this.NtHeaders.FileHeader.NumberOfSections - 1);

            IMAGE_SECTION_HEADER LastSection = ByteArrayToStructure<IMAGE_SECTION_HEADER>(this.PeBuffer
                .Skip(TargetHeaderOffset)
                .Take(SectionHeaderSize)
                .ToArray());
